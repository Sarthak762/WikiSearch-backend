...
def docsToFiles(foldername, language):
    ...
    for filename in os.listdir(input_path):
        with open(input_path + '/' + filename, encoding="utf8") as in_file:
            lines = [[line.rstrip('\n')] for line in in_file]
        ...
        for line in lines:
            if '<doc id=' in line[0]:
                ...
                with open(folder_path + docID + '.txt', 'w', encoding="utf8") as f:
...
def indexing(backup):
    ...
    for k in backup.keys():
        docID = k
        with open(folder_path + docID + '.txt', 'r', encoding="utf8") as inputs:
        ...
        for token in line:
            if token not in index_dic.keys():
                ...
                if docID in index_dic[token].keys():
                    ...
                    index_dic[token][docID] = 1
    with open('index.txt', 'w', encoding="utf8") as index_output, open('positions.txt', 'w', encoding="utf8") as position_output:
...
def doc_vector(ind, back, pos, docLen):
    ...
    with open('vectorSpace.txt', 'w', encoding="utf8") as vec, open('mapping.txt', 'w', encoding="utf8") as mapp:
